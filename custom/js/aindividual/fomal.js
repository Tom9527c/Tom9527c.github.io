...
$.ajax({
  ...
  data: {
    key: 'ZR3BZ-AB633-VYY36-OL2J7-BPOP3-SIBPV',  // 这里要写你的KEY!!! QGHBZ-K7QKP-37IDO-L2HNC-WYIH6-O5BL4 ZR3BZ-AB633-VYY36-OL2J7-BPOP3-SIBPV
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(113.75842 , 23.02731, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  var grt = new Date("09/13/2023 18:20:25"); //此处修改你的建站时间或者网站上线时间
  now2.setTime(now2.getTime() + 250);
  var days = (now2 - grt) / 1000 / 60 / 60 / 24;
...
function createtime() {
  ...
  var start = new Date("09/13/2023 18:00:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(2340000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("09/13/2023 18:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://sourcebucket.s3.bitiful.net/badge/F小屋-科研摸鱼中.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> </div>`
      : `<img class='boardsign' src='https://sourcebucket.s3.bitiful.net/badge/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> </div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://sourcebucket.s3.bitiful.net/img/dm2.webp);
    --darkmode-bg: url(https://sourcebucket.s3.bitiful.net/img/dm7.webp);
    --mobileday-bg: url(https://sourcebucket.s3.bitiful.net/img/mb7.webp);
    --mobilenight-bg: url(https://sourcebucket.s3.bitiful.net/img/mb8.webp);
    --anzhiyu-background: url(https://sourcebucket.s3.bitiful.net/img/dm2.webp);
  }`;
}
//   #【必选】默认背景 
